...
describe('PageListStore', () => {
  let store: TestPageListStore<any, any, any>;

  class TestPageListStore<TContent, TData, TList> extends PageListStore<TContent, TData, TList> {}

  beforeEach(() => {
    TestBed.configureTestingModule({});

    store = new TestPageListStore(
      { valor: Math.random() },
      { dado: Math.random() },
      { items: [{ dado: Math.random() }] },
    );
  });
  ...
  it('deve expor o observable de lista', () => {
    const resultado = store.list$;
